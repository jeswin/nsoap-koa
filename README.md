  ...
  greeting(id) {
    return (ctx) => {
      if (ctx.method === "POST") {
        ...
        ctx.throw(404, "Sorry, nothing to see here.")
      }
    }
  },
}
...
const myApp = {
  streamData() {
    return (ctx) => {
      ctx.body = fs.createReadStream("/some/path/to/file");
    }
  },
}
```
...
Passes the KoaJS context (request and response object) as a method parameter.
By default the context is the object ```{ ctx, isContext: () => true }``` where ctx is the KoaJS context. 
...
function createContext(context) {
  return { ...context, IS_ADMIN: context.ctx.request.ip === "127.0.0.1" }
}
...
### getBody(ctx: KoaContext) : Object

Allows an app to alter the body parameter dictionary passed into the router.
If getBody is not defined, the body is assumed to be ctx.body; which is what middleware like "bodyparser" do.
...
function getBody(ctx) {
  return { ...ctx.request.body, newParam: 1, anotherParam: "TWO" }
}
...
This is the same dictionary that was previously constructed via getBody() or in its absense ctx.request.body.
